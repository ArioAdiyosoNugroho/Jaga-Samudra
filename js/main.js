...
document.addEventListener('DOMContentLoaded', function() {
  ...
  window.addEventListener('scroll', function() {
    ...
    parallaxLayers.forEach(layer => {
      const depth = parseFloat(layer.getAttribute('data-depth')) || 0.1;
      const parent = layer.closest('.tab-image');
      const parentRect = parent.getBoundingClientRect();
      
      // Hitung posisi relatif terhadap viewport
      const relativePosition = parentRect.top / viewportHeight;
      
      // Batasi pergerakan parallax
      let moveY = scrollPosition * depth * relativePosition;
      
      // Batasi pergerakan maksimal (optional)
      moveY = Math.min(moveY, 100); // Maksimal bergerak 100px
